...
const SURVEY: SurveyStep[] = [
  { title: "생활", description: "제품이 실제 생활에서 얼마나 중요한지 확인합니다.", questions: [
  ...
  { title: "재정", description: "소득을 캐묻지 않고, 실제 구매 가능한 범위를 봅니다.", questions: [
  ...
  { title: "취향", description: "사양표가 아니라 실제 선택 기준의 우선순위를 잡습니다.", questions: [
  ...
  { title: "보유 제품", description: "새 제품보다 먼저, 지금 가진 제품으로 해결 가능한지 봅니다.", questions: [
  ...
  { title: "환경", description: "사용 장소와 물리적 제약을 반영합니다.", questions: [
  ...
  { title: "과거 구매", description: "후회와 만족 패턴을 다음 결정의 교정 데이터로 씁니다.", questions: [
  ...
  { title: "미래 계획", description: "지금만 보지 않고 다음 3~12개월 계획을 함께 봅니다.", questions: [
  ...
  { title: "구매 성향", description: "바이저가 답을 제시하는 방식까지 개인화합니다.", questions: [
  ...
  { title: "카테고리", description: "카테고리 진입 시 필요한 추가 질문만 이어서 묻습니다.", questions: [
...
export function UserModelClient() {
  ...
  function clearState() {
    ...
    setHasAnalysis(false);
    setConfirmed(false);
    try { localStorage.removeItem("buysor-preview-state"); } catch {}
  }
  ...
  const rows: Array<[string, string]> = [
    ["현재 제품", profile.currentProduct], ["주요 불만", profile.painPoint], ["편안한 예산", profile.budget],
    ["중고 구매", profile.usedOkay], ["구매 긴급도", profile.urgency], ["예상 사용기간", profile.duration], ["미래 계획", profile.futurePlan],
  ];
        ...
        <section className={styles.profilePanel}>
          <article className={styles.editorCard}>
            ...
            <textarea
              ...
              value={text}
              onChange={(event) => { setText(event.target.value); setHasAnalysis(false); setConfirmed(false); }}
            ...
            <div className={styles.exampleRow}>
              ...
              <span>예시는 저장되지 않으며 직접 수정할 수 있습니다.</span>
            </div>
