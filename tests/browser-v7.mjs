...
try{
 for(const width of [1440,768,390]){
  ...
  await page.screenshot({path:`browser-evidence/home-${width}.png`,fullPage:true});
  ...
  await page.screenshot({path:`browser-evidence/credits-${width}.png`,fullPage:true});
  ...
  await page.screenshot({path:`browser-evidence/membership-${width}.png`,fullPage:true});
  if(width===1440){await page.goto(origin,{waitUntil:'networkidle'});await page.evaluate(()=>localStorage.setItem('buysor-theme','dark'));await page.reload({waitUntil:'networkidle'});await page.screenshot({path:'browser-evidence/home-dark.png',fullPage:true});}
 ...
 await writeFile('browser-evidence/results.json',JSON.stringify({results,status,unauthenticatedDecision:denied.status,disabledTierSwitch:preview.status},null,2));
